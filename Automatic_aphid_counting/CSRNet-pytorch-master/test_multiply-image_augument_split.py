...
img_paths = './Shanghai/part_A_final/test_data/images/'
h5_paths = './Shanghai/part_A_final/test_data/ground_truth/'
save_path = './result/aphids_images_63_second_converse_augument-trainingset/test_result/'
#save_path = './Shanghai/part_A_final/val_data/ground_truth/'


image_list=[]
for filename in os.listdir(img_paths):
    image_list.append((filename.rstrip('.jpg')))#只保存名字，去除后缀.png
...
for img in image_list:
    ...
    img_path = img_paths+img_name+'.jpg'
    h5_path = h5_paths+img_name+'.h5'
    print('img_path,h5_path',img_path,h5_path)
    ...
    temp = np.asarray(
        output_src.detach().cpu().reshape(output_src.detach().cpu().shape[2], output_src.detach().cpu().shape[3]))
    print('output.detach().cpu().shape[2],output.detach().cpu().shape[3]', output_src.detach().cpu().shape[2],
    ...
    plt.subplot(233), plt.title("Pre_density map_"+str(int(output)),fontsize=font_size), plt.axis('off')
    plt.imshow(temp, cmap = c.jet)
    ...
    #save
    plt.ion()
    plt.savefig(save_path+img_name+'_result.jpg')
    #plt.pause(3)
    plt.close()
